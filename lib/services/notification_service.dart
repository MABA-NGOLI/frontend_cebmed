...
// Regroupe toute la logique de notification: notifications locales, FCM et rappels RDV.
class NotificationService {
  ...
  // Initialise les notifications locales et branche Firebase Cloud Messaging.
  ...
  // ── FCM ───────────────────────────────────────────────
  // Configure FCM: permission, renouvellement du token et messages reçus au premier plan.
  ...
  // Envoie le token FCM au backend pour qu'il puisse pousser des notifications à cet appareil.
  static Future<void> _sendTokenToBackend(String token) async {
    try {
      ...
      debugPrint(
        '[FCM] Authorization present: ${hdrs.containsKey("Authorization")}',
      );
  ...
  // Programme une notification locale avant un rendez-vous médical.
  ...
  // À appeler après connexion ou reprise d'app pour associer l'appareil au compte connecté.
  static Future<void> syncFcmToken() async {
    try {
      debugPrint(
        '[FCM] syncFcmToken: start, Firebase apps: ${Firebase.apps.map((a) => a.name).toList()}',
      );
      ...
      debugPrint(
        '[FCM] syncFcmToken: token=${token == null ? "NULL" : "${token.substring(0, 20)}..."}',
      );
      if (token != null) {
  ...
  // Annule le rappel local associé à un rendez-vous supprimé ou modifié.
